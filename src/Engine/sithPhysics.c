...
void sithPhysics_ThingPhysGeneral(sithThing *pThing, flex_t deltaSeconds)
{
    ...
    {
        // Apply configurable sector thrust scale factors: horizontal (x/y) and vertical (z)
        rdVector3 scaledThrust;
        scaledThrust.x = pThing->sector->thrust.x * jkPlayer_sectorThrustScaleH;
        scaledThrust.y = pThing->sector->thrust.y * jkPlayer_sectorThrustScaleH;
        scaledThrust.z = pThing->sector->thrust.z * jkPlayer_sectorThrustScaleV;
    ...
    for (int i = (int)framesToApply; i > 0; i--)
    {
        ...
        if ( player->physicsParams.mass != 0.0 )
        {
                ...
                && !(player->physicsParams.physflags & SITH_PF_NOTHRUST))
            {
                // Apply configurable sector thrust scale factors: horizontal (x/y) and vertical (z)
                rdVector3 scaledThrust;
                scaledThrust.x = player->sector->thrust.x * jkPlayer_sectorThrustScaleH;
                scaledThrust.y = player->sector->thrust.y * jkPlayer_sectorThrustScaleH;
                scaledThrust.z = player->sector->thrust.z * jkPlayer_sectorThrustScaleV;
...
// MOTS added: weapon tracking?
    ...
    if (pThing->physicsParams.mass != 0.0 && (pThing->sector->flags & SITH_SECTOR_HASTHRUST) && !(pThing->physicsParams.physflags & SITH_PF_NOTHRUST))
    {
        ...
        if ( pThing->sector->thrust.z * jkPlayer_sectorThrustScaleV > sithWorld_pCurrentWorld->worldGravity * pThing->physicsParams.mass )
        {
            sithThing_DetachThing(pThing);
            rdVector_Zero3(&pThing->physicsParams.addedVelocity);
            rdVector_Zero3(&out);
